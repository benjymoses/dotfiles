...
- **Personas** — `implementer` / `spec-validator` / `code-reviewer` are custom agents in `~/.claude/agents/` (source: dotfiles). Spawn them by `subagent_type`; each carries its own `model:` frontmatter. They work as teammate types too.
- **Model on generic spawns: `model: "sonnet"` (shorthand)** — personas carry their own model; every OTHER spawn (Explore, Plan, ad-hoc) passes the shorthand. EU Bedrock needs region-prefixed IDs set via env vars; only the shorthand resolves correctly. Project-specific dispatch detail (e.g. an OpenSpec apply loop) belongs in that project's schema/skill, not here.
- **Wait, don't poll** — teammate/subagent messages and completion notifications arrive automatically; after dispatching, end the turn. Never busy-wait with `sleep` loops (a sleep-guard hook denies them).
