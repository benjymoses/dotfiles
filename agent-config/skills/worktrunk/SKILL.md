...
## Local note (added by dotfiles, not upstream)

The `reference/*.md` files listed above are **not vendored here**. Upstream syncs
them from worktrunk.dev, so a local copy would go stale. Use these instead:

- `wt <command> --help` — authoritative and always matches the installed binary
- `wt config show` — actual config paths, shell integration and plugin status
- `wt hook show` — the hooks that will actually run, user and project
- <https://worktrunk.dev/llms.txt> — LLM-readable full documentation, on demand

This machine's setup:

- User config (all repos): `~/.config/worktrunk/config.toml`, symlinked from
  `~/dotfiles/.config/worktrunk/config.toml`
- User hooks run for every repo: `pre-start` copies gitignored files via
  `wt step copy-ignored`, then installs Node dependencies if a `package.json`
  exists, honouring the lockfile and defaulting to pnpm
- Per-repo overrides go in `<repo>/.config/wt.toml`

Source: <https://github.com/max-sixty/worktrunk> (`skills/worktrunk/SKILL.md`),
MIT OR Apache-2.0. Refresh with:

    curl -fsSL https://raw.githubusercontent.com/max-sixty/worktrunk/main/skills/worktrunk/SKILL.md \
      -o ~/dotfiles/.kiro/skills/worktrunk/SKILL.md
    # then re-append this note
